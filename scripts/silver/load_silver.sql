--- Truncate Tables and then insert transformed and cleansed data into the tables 
 
EXEC silver.load_silver; ---- execute the stored procedure
...
BEGIN
	...
	CASE WHEN UPPER(TRIM(cst_material_status)) = 'S' THEN 'Single'
		...
		ELSE 'n/a'
	END cst_material_status, -- Normalize data marital values

	CASE WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female'
		WHEN upper(TRIM(cst_gndr)) = 'M' THEN 'Male' 
		ELSE 'n/a'
	END cst_gndr, -- Normalize gender values
	cst_create_date
	FROM(
		SELECT  *,
	...
	SELECT 
		...
		--Calculate End daate as one day before next start date
